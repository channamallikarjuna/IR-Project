...
loss, acc = new_model.evaluate(xtest_pad, test_label_seq, verbose=2)
...
for x in xtrain_pad:
	...
	feature = new_model.predict(x)[0]
	features.append(feature / np.linalg.norm(feature)) 

for i in range(5):
	print(features[i])
feature_path = os.getcwd()+"/fea"+".npy"
...
for x in xtest_pad:
	feature = new_model.predict(x)[0]
	test_features.append(feature / np.linalg.norm(feature)) 
feature_path_test = os.getcwd()+"/fea_test"+".npy"
np.save(feature_path_test, test_features)
